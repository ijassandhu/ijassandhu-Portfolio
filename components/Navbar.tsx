...
const NAV_HEIGHT = 64;
const MIN_VISIBLE_SECTION = 80;
const trackedSectionIds = [...navLinks.map((link) => link.href.slice(1)), "contact"];
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const updateNavState = () => {
      ...
      setActiveSection(bestVisibleHeight > MIN_VISIBLE_SECTION ? bestSection : "");
    };
          ...
          <div className="flex items-center h-[64px]">
            ...
            <div className="flex-1 flex justify-end">
              <a
                ...
                className="hidden md:inline-flex items-center px-4 py-[7px] text-[13px] font-semibold text-white rounded-full transition-all duration-200"
                style={{
                  backgroundColor: activeSection === "contact" ? "#111827" : "#4f858b",
                }}
                onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.backgroundColor = "#3a666b"; }}
                onMouseLeave={(e) => {
                  (e.currentTarget as HTMLElement).style.backgroundColor =
                    activeSection === "contact" ? "#111827" : "#4f858b";
                }}
              >
                Contact Me
              </a>
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <a
                    ...
                    href={link.href}
                    onClick={(e) => { e.preventDefault(); handleLink(link.href); }}
                    className={`px-4 py-2.5 text-[14px] rounded-xl transition-colors ${
                      isActive
                        ? "bg-black/[0.05] font-semibold text-[#111827]"
                        : "font-medium text-[#4b5563] hover:bg-gray-50 hover:text-[#111827]"
                    }`}
                  >
                    {link.label}
                  </a>
